...
season17 %>% 
  ...
  title = "FPL points to minutes played ratio vs Total FPL points for
...
fpl_point_rank <- function(df) {
  df %>% 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    labs(title = sprintf("The top 10 players with 
                         the highest FPL points in %s", 
...
labs(title = sprintf("A frequency polygon graph of the ages of Premier League players with
...
sel_fpl <- function(df) {
  df %>% 
    ...
    geom_point(aes(color = position_index), position = "jitter") + 
    labs(
    title = sprintf("A graph of FPL points vs selected by percent in the
                    season %s", 
